...
var jasmine = require('./jasmine-1.3.1').jasmine;
...
function filterStackTrace(stackTrace) {
  return stackTrace.split('\n').filter(function(stack) {
    return stack.search('phantomjs:\/\/bootstrap\.js|jasmine-1\.3\.1\.js') === -1;
  }).join('\n');
}
...
jasmineNode.TerminalReporter = function(config) {
  ...
  this.stackFilter_ = config.stackFilter || function(t) {
    return t;
  };
};
...
jasmineNode.TerminalReporter.prototype = {
  ...
  reportFailures_: function() {
    ...
    for (var i = 0; i < this.failures_.length; i++) {
      ...
      if (this.includeStackTrace_ && failure.stackTrace) {
        this.printLine_('');
        this.printLine_('   Stacktrace:');
        this.printLine_('');
        this.print_('     ' + this.stackFilter_(filterStackTrace(failure.stackTrace)));
      }
    }
  },
